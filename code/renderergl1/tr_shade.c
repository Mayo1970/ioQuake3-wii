...
void R_DrawElements( int numIndexes, const glIndex_t *indexes ) {
	...
	if ( primitives == 2 ) {
		...
		if ( backEnd.refdef.rdflags & RDF_NOWORLDMODEL ) {
			...
			if ( s_drawCount < 16 ) {
				wii_diag("[draw] #%d glDrawElements ni=%d nv=%d mv=[%.1f,%.1f,%.1f,%.1f]\n",
				         s_drawCount, numIndexes, tess.numVertexes,
				         backEnd.or.modelMatrix[12], backEnd.or.modelMatrix[13],
				         backEnd.or.modelMatrix[14], backEnd.or.modelMatrix[15]);
				s_drawCount++;
			}
		}
#endif
