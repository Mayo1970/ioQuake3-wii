	...
	for (i = 0, drawSurf = drawSurfs ; i < numDrawSurfs ; i++, drawSurf++) {
		...
		if ( shader != NULL && ( shader != oldShader || fogNum != oldFogNum || dlighted != oldDlighted
			...
			if ( backEnd.refdef.rdflags & RDF_NOWORLDMODEL ) {
				...
				if (s_shaderCount < 16) {
					wii_diag("[3d] shader#%d ent=%d name=%s\n",
					         s_shaderCount, entityNum,
...
#ifdef WII_DEBUG
	{
		static int s_surfCount = 0;
		if (s_surfCount < 4) {
			wii_diag("[3d] scene#%d refdef(x=%d y=%d w=%d h=%d) vp(x=%d y=%d w=%d h=%d) rdflags=0x%x\n",
			         s_surfCount,
			         cmd->refdef.x, cmd->refdef.y,
			         cmd->refdef.width, cmd->refdef.height,
			         cmd->viewParms.viewportX, cmd->viewParms.viewportY,
			         cmd->viewParms.viewportWidth, cmd->viewParms.viewportHeight,
			         cmd->refdef.rdflags);
			s_surfCount++;
		}
	}
