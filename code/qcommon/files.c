...
qboolean FS_PakIsPure( pack_t *pack ) {
	...
	if ( fs_numServerPaks ) {
		for ( i = 0 ; i < fs_numServerPaks ; i++ ) {
			...
			// Note: pk3 with same checksum but different name would validate too (not an exploit)
...
// Check if file exists in current gamedir (no path search). Detects overwrite on write ops.
// Pair with FS_FOpenFileWrite.
...
int	FS_GetModList( char *listbuf, int bufsize ) {
	...
	*listbuf = 0;
	...
	for (i = 0; i < nPotential; i++) {
		...
		// Valid mod: needs at least one .pk3 or .pk3dir. Try all OS paths.
...
void FS_Shutdown( qboolean closemfp ) {
	...
	wii_diag("FS_Shutdown: closing file handles\n");
	...
	wii_diag("FS_Shutdown: freeing searchpaths\n");
	// free everything
	for(p = fs_searchpaths; p; p = next)
	{
		...
		if(p->pack) {
			wii_diag("FS_Shutdown: FreePak %s\n", p->pack->pakFilename);
			FS_FreePak(p->pack);
		}
		if (p->dir)
			Z_Free(p->dir);
	...
	for (nFlags = FS_CGAME_REF; nFlags; nFlags = nFlags >> 1) {
		...
		for ( search = fs_searchpaths ; search ; search = search->next ) {
			...
			if ( search->pack && (search->pack->referenced & nFlags)) {
				...
				if ( compat ) {
					int pakNum;
					/* proto-43: only report stock pak0-pak2; skip Wii-custom paks */
...
void FS_Restart( int checksumFeed ) {
	...
	wii_diag("FS_Restart: FS_Shutdown\n");
	// free anything we currently have loaded
	FS_Shutdown(qfalse);

	wii_diag("FS_Restart: set checksumFeed\n");
	// set the checksum feed
	fs_checksumFeed = checksumFeed;

	wii_diag("FS_Restart: FS_ClearPakReferences\n");
	// clear pak references
	FS_ClearPakReferences(0);

	wii_diag("FS_Restart: FS_Startup\n");
	// try to start up normally
	FS_Startup(com_basegame->string);
	wii_diag("FS_Restart: FS_Startup done\n");

	/* FS_CheckPak0 already ran at FS_InitFilesystem; skipping on restart
	 * avoids a false ERR_FATAL when mod paks are present alongside baseq3. */

	wii_diag("FS_Restart: FS_ReadFile default.cfg\n");
	// if we can't find default.cfg, assume that the paths are
	// busted and error out now, rather than getting an unreadable
	// graphics screen when the font fails to load
	if ( FS_ReadFile( "default.cfg", NULL ) <= 0 ) {
	...
	wii_diag("FS_Restart: default.cfg found\n");

	lastGameDir = ( lastValidGame[0] ) ? lastValidGame : lastValidComBaseGame;

	wii_diag("FS_Restart: gamedir check lastGameDir=%s cur=%s\n", lastGameDir, FS_GetCurrentGameDir());
