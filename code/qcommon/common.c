...
void Com_GameRestart(int checksumFeed, qboolean disconnect)
{
	...
	if(!com_gameRestarting && com_fullyInitialized)
	{
		...
		wii_diag("GameRestart: FS_Restart\n");
		...
		if(com_gameClientRestarting)
		{
			wii_diag("GameRestart: CL_Init\n");
			CL_Init();
			wii_diag("GameRestart: CL_StartHunkUsers\n");
			CL_StartHunkUsers(qfalse);
			wii_diag("GameRestart: CL_StartHunkUsers done\n");
		}
		wii_diag("GameRestart: done\n");
		com_gameRestarting = qfalse;
		com_gameClientRestarting = qfalse;
	}
}
...
void Com_ReadCDKey( const char *filename ) {
	...
	if ( !f ) {
#if defined(GEKKO) && !defined(DEDICATED) && !defined(STANDALONE)
...
void Com_Frame( void ) {
	...
	if(!com_timedemo->integer)
	{
		...
		else
		{
			...
			// Menus have no world geometry to bottleneck on, so let them run at
			// 60 - only ever raises the cap, never below the in-game value.
			if(CL_InMenu())
			{
